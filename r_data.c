...
void R_InitTextures (void)
{
    ...
    //!!printf("patchlookup[%d] = %d", i, patchlookup[i]);
    }
    Z_Free (names);
...
#if 0
	_memcpy (texture->name, mtexture->name, sizeof(texture->name));
#else
	{ char *src; char *dst;
	...
	for (j=0 ; j<texture->patchcount ; j++, mpatch++, patch++)
	{
        ...
        //!!printf("patchlookup[%d], mpatch offset from start=%u", SHORT(mpatch->patch),
        //!!    ((char*)mpatch - (char*)(&mtexture->patches[0])));
	    if (patch->patch == -1)
	    {
		I_Error ("R_InitTextures: Missing patch in texture %s",
			 texture->name);
	    }
        //!!printf("Texture:(%d/%d) %s ", j, texture->patchcount, texture->name);
	}		
