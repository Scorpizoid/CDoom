...
#if defined(linux) || defined(__BEOS__) || defined(__SVR4) || defined(__SYMBIAN32__) //!!
void strupr (char* s)
{
...
void W_AddFile (char *filename)
{
    ...
    printf("Alloc fileinfo");
	header.numlumps = LONG(header.numlumps);
	header.infotableofs = LONG(header.infotableofs);
	length = header.numlumps*sizeof(filelump_t);
    printf("Alloca %d", length);
    ...
    {
    //!!printf("Initialize lump %d of %d", i, numlumps);
	lump_p->handle = storehandle;
	lump_p->position = LONG(fileinfo->filepos);
	lump_p->size = LONG(fileinfo->size);
	strncpy (lump_p->name, fileinfo->name, 8);
    }
	printf("%d lumps initalized", numlumps);
    if (reloadname)
	fclose (handle);
}
...
void W_Profile (void)
{
    ...
    for (i=0 ; i<numlumps ; i++)
    {
	_memcpy (name,lumpinfo[i].name,8);
